...
echo "$($PSQL"SELECT name FROM teams WHERE team_id IN (SELECT winner_id FROM games WHERE round LIKE 'Eighth%' AND year=2014) OR team_id IN (SELECT opponent_id FROM games WHERE round LIKE 'Eighth%' AND year=2014) ORDER BY name;")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL"SELECT DISTINCT(name) FROM teams, games WHERE teams.team_id=games.winner_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL"SELECT games.year, teams.name FROM teams, games WHERE games.round='Final' AND teams.team_id=games.winner_id ORDER BY year")"
